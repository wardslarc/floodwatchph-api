...
router.post('/register', validateSignup, async (req, res, next) => {
  try {
    ...
    // Send welcome email
    try {
      await emailService.sendWelcomeEmail(user); // Updated call
    ...
    try {
      ...
      // Send 2FA code
      const emailResult = await emailService.send2FACode(user.email, verificationCode); // Updated call
  ...
  try {
    ...
    try {
      ...
      if (emailResult.success) {
        ...
        logger.error('Failed to send 2FA code for auto-enable:', { 
          userId: user._id, 
          error: emailResult.error 
        });
      }
  ...
  try {
    ...
    try {
      ...
      if (emailResult.success) {
        ...
        logger.error('Failed to resend 2FA code:', { 
          userId: user._id, 
          error: emailResult.error 
        });
      }
